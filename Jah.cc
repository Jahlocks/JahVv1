...
local Jahseh = Instance.new("ScreenGui")
...
Jahseh.Name = "Jahseh"
Jahseh.Parent = game.CoreGui
Jahseh.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
...
Logo.Image = "rbxassetid://8652665149"
...
TextButton.MouseButton1Click:Connect(
    function()
        state = not state
        if not state then
            TextButton.Text = "Jah.cc ON"
            CamlockState = true
            enemy = FindNearestEnemy()
        else
            TextButton.Text = "Jah.cc OFF"
            CamlockState = false
            enemy = nil
        end
    end
